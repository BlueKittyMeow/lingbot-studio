...
OUT_GLB = '/mnt/e/wsl_deploy/ch_max_mesh_lily2.glb'
...
best_w = np.zeros(len(V)); best_c = np.full((len(V),3), 0.15)
for i, f in enumerate(files):
    ...
    best_c[better] = col[better]
    if i % 60 == 0: print(f'  {i}/{len(files)}', flush=True)

colors = best_c.copy()
colors[best_w < 1e-6] = [0.15, 0.16, 0.2]
# saturation boost for vividness
g = colors.mean(1, keepdims=True); colors = np.clip(g + (colors - g) * 1.6, 0, 1)
