...
SRC='/home/bluekitty/renders/c20/catacombs2_crop'; OUT='/home/bluekitty/renders'; PER=45000
...
for f in files:
    ...
    ext=d['extrinsic']; R=ext[:3,:3]
    ups.append(-R[1,:]); fwds.append(R[2,:])
    world,_,mask=depth_to_world_coords_points(depth,ext,d['intrinsic'])
...
u=np.mean(ups,0); u=u/np.linalg.norm(u)
f=np.mean(fwds,0); f=f-(f@u)*u; f=f/np.linalg.norm(f)
r=np.cross(f,u); r=r/np.linalg.norm(r)
Ralign=np.stack([r,u,-f])
if np.linalg.det(Ralign)<0: Ralign[0]=-Ralign[0]
Rx=np.diag([1.0,-1.0,-1.0])
Rfinal=(Rx@Ralign).astype(np.float32)
print('up',np.round(u,3),'detR',round(float(np.linalg.det(Rfinal)),3),flush=True)
med=np.median(pts,0); print('base',len(pts),flush=True)
def save(p,c,name):
    p=((p-med)@Rfinal.T).astype(np.float32)
...
save(pts,col,'catacombs20l_max_fused.ply')
for tgt,name in [(6_000_000,'catacombs20l_6m_fused.ply'),(3_000_000,'catacombs20l_3m_fused.ply'),(1_500_000,'catacombs20l_light_fused.ply')]:
