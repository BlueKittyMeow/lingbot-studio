...
SRC='/home/bluekitty/renders/courthouse_sparse/courthouse'; OUT='/home/bluekitty/renders/courthouse_sparse_splat.ply'
ITERS=int(sys.argv[1]) if len(sys.argv)>1 else 4000
INIT_N=int(sys.argv[2]) if len(sys.argv)>2 else 800000
...
N=len(pts); print(f"cams {F}  img {H}x{W}  init gaussians {N}",flush=True)
...
for it in range(ITERS):
    i=np.random.randint(F); loss=torch.abs(render(i)-imgs[i]).mean()
    opt.zero_grad(); loss.backward(); opt.step()
    if it%100==0 or it==ITERS-1: print(f"it {it:5d}  loss {loss.item():.4f}  ({time.time()-t0:.0f}s)",flush=True)
